...
class Language(Base):
    ...
    native_name: Mapped[str] = mapped_column(String(120), nullable=True)
...
class CourseI18n(Base):
    ...
    subtitle: Mapped[str] = mapped_column(String(220), nullable=True)
...
class PathUnit(Base):
    ...
    guidebook_payload: Mapped[dict] = mapped_column(JSON, nullable=True)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False)
...
class Question(Base):
    ...
    hint_payload: Mapped[dict] = mapped_column(JSON, nullable=True)
    created_by: Mapped[int] = mapped_column(ForeignKey("users.id"), nullable=True)
    updated_by: Mapped[int] = mapped_column(ForeignKey("users.id"), nullable=True)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False)
...
class UserQuestionAttempt(Base):
    ...
    ai_feedback_id: Mapped[UUID] = mapped_column(Uuid(as_uuid=True), ForeignKey("ai_explanations.id"), nullable=True)
...
class ReviewQueueItem(Base):
    ...
    id: Mapped[UUID] = mapped_column(Uuid(as_uuid=True), primary_key=True, default=uuid4)
    user_id: Mapped[int] = mapped_column(ForeignKey("users.id", ondelete="CASCADE"), index=True)
    source_lesson_id: Mapped[int] = mapped_column(ForeignKey("lessons.id", ondelete="CASCADE"), nullable=True)
...
class AIExplanation(Base):
    ...
    question_id: Mapped[UUID] = mapped_column(Uuid(as_uuid=True), ForeignKey("questions.id"), nullable=True, index=True)
    ...
    cefr_level: Mapped[str] = mapped_column(String(8), nullable=True)
